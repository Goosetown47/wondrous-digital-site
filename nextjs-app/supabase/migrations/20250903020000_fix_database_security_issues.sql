...
-- ========================================
-- 3. FIX: Add search_path to all functions
-- ========================================
-- All functions need explicit search_path to prevent SQL injection vulnerabilities
...
-- Function 9: cleanup_old_pending_payments
...
BEGIN
  ...
  DELETE FROM pending_stripe_payments
  WHERE created_at < NOW() - INTERVAL '7 days'
    AND status = 'pending';
END;
$$;

-- Function 10: process_pending_payment
...
AS $$
BEGIN
  UPDATE pending_stripe_payments
  SET 
    status = p_status,
...
-- Function 11: get_project_access_for_account (if exists)
-- First check if function exists before recreating
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'get_project_access_for_account'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    CREATE OR REPLACE FUNCTION public.get_project_access_for_account(p_account_id uuid)
...
-- Function 12: update_account_billing_dates (if exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'update_account_billing_dates'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    ...
    AS $func$
    BEGIN
      -- Update billing-related timestamps when subscription changes
...
BEGIN
  ...
  RAISE NOTICE '- Added search_path to all 15 functions';
END $$;
...
/**
 * Get CSS font-family value with proper quoting and fallbacks
...
export function getFontFamilyCSS(fontName: string): string {
  ...
  // Quote font names with spaces using double quotes, others use single quotes
  const quotedName = fontName.includes(' ') ? `"${fontName}"` : `'${fontName}'`;
  return `${quotedName}, ${font.fallback}`;
}

/**
 * Load Google Fonts with callback when loaded
 * @param fonts Array of font names to load
 * @param onLoad Callback function called when fonts are loaded
 */
export function loadGoogleFontsWithCallback(
  fonts: string[],
  onLoad?: () => void
): void {
  if (typeof window === 'undefined') return; // Only run in browser
  ...
  if (existingLink) {
    // Font already loaded or loading, call callback after short delay
    if (onLoad) {
  ...
  if (onLoad) {
    link.onload = () => {
      // Give fonts a moment to be registered with document.fonts
      setTimeout(onLoad, 100);
    };
  }

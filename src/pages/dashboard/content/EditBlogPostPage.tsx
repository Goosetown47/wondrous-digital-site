...
import { editorJsToMarkdown, markdownToEditorJs } from '../../../utils/editorjs-converter';
...
const EditBlogPostPage = () => {
  ...
  const [originalPublishDate, setOriginalPublishDate] = useState<string>('');
  
  // UI state
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [slugEdited, setSlugEdited] = useState(true); // Always consider slug edited in edit mode
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        // Handle publish date
        const publishAt = new Date(post.published_at);
        setOriginalPublishDate(publishAt.toISOString());
        
        // Format date to local YYYY-MM-DDThh:mm format for datetime-local input
        const formattedDate = publishAt.toISOString().slice(0, 16);
        setPublishDate(formattedDate);
        
        // If publish date is in the future, set publishImmediately to false
        setPublishImmediately(publishAt <= new Date());
  ...
  useEffect(() => {
    // Only initialize once when content is ready
    if (editorHolderRef.current && !editorRef.current && !loading && !hasInitialized.current) {
      const projectId = selectedProject?.id || 'temp';
      
      // Convert markdown to EditorJS data
      const editorData = markdownToEditorJs(markdownContent);
      
      // Initialize editor with the loaded content
  ...
  }, [loading, selectedProject?.id]); // Initialize when loading is done and project is ready

  // Generate slug from title (only if title changes and slug hasn't been manually edited)
  useEffect(() => {
    if (title && !slugEdited) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!postId) {
      setError('Post ID is missing. Please go back and try again.');
      return;
    }
    ...
    try {
      ...
      // Don't change the published date if it hasn't been modified
      // This preserves the original publishing date for already published posts
      if (!publishImmediately && publishDate === originalPublishDate.slice(0, 16)) {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          // project_id: Don't change project_id, keep the post associated with its original project
      ...
      navigate('/dashboard/content/blog', { 
        state: { 
          success: true,
          message: `Blog post "${data.title}" updated successfully!` 
        } 
      });
    } catch (err: any) {
  ...
  if (!selectedProject) {
    return (
      <div className="bg-white p-6 shadow-sm rounded-lg">
        ...
        <p className="text-gray-600">Please select a project to edit a blog post.</p>
        <button
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="bg-white shadow-sm rounded-lg p-8 flex items-center justify-center">
          ...
          <span className="ml-3 text-gray-700">Loading post content...</span>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-white shadow-sm rounded-lg overflow-hidden">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  <label htmlFor="publishNow" className="ml-2 block text-sm text-gray-900">
                    Publish immediately (updates original date for published posts)
                  </label>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <label htmlFor="publishLater" className="ml-2 block text-sm text-gray-900">
                    Set specific publication date and time
                  </label>
                </div>
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
            ...
            <button
              ...
              className={`px-4 py-2 bg-primary-pink text-white rounded-lg hover:bg-primary-dark-purple transition-colors duration-200 flex items-center ${(saving || loading) ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {saving ? (
                <>
